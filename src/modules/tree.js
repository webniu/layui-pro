...
layui.define('form', function(exports){
    ...
    layui.each(data, function(index, item){
        ...
        ,'<div class="layui-tree-entry">'
          ,'<div class="layui-tree-main">'
            ...
            ,function(){
              if(item.num){
              if(options.isJump && item.href){
                return '<a href="'+ item.href +'" target="_blank" class="'+ ELEM_TEXT +'"> ('+(item.num)+') </a>';
              }else{
                return '<span class="encloseddata '+ ELEM_TEXT + (item.disabled ? ' '+ DISABLED : '') +'"> ('+(item.num)+') </span>';
              }
            }
      ...
      function(){
        if(!hasChild){
          ...
          if(!options.mdefinend) return '';
          
          var arr = ['<div class="layui-btn-group mtree-btnGroup">'];
          
          if(typeof options.mdefinend === 'object'){
            layui.each(options.mdefinend, function(i, val){
              arr.push('<i class="layui-icon icon-btns '+val.class+' icon-'+val.class+'" data-type="'+val.title+'" title='+val.title+' data-id="'+ item[customName.id] +'"></i>' || '')
            });
            return arr.join('') + '</div>';
          }
        }else{
      ...
      //操作节点
      options.mdefinend && that.operate2(entryDiv, item);
  ...
  Class.prototype.setChecked = function(checkedId){
    ...
    // 节点操作
    Class.prototype.operate2 = function(elem, item){
      var that = this;
      var options = that.config;
      var entry = elem.children('.'+ ELEM_ENTRY);
      var elemMain = entry.children('.'+ ELEM_MAIN);
  
      entry.children('.mtree-btnGroup').on('click', '.layui-icon', function(e){
        layui.stope(e);  // 阻止节点操作
        // 复选框点击产生的回调
